...
export class CrossmarkAdapter implements WalletAdapter {
  ...
  // Crossmark supports seamless network switching - the extension manages connections
   ...
   * Crossmark manages the XRPL connection internally, so we just update the stored network
   */
  async switchNetwork(network: NetworkInfo): Promise<AccountInfo> {
    ...
    // Crossmark handles the actual XRPL node connection internally
    this.currentAccount = {
      ...this.currentAccount,
      network,
    };
